...
sys_exofork(void)
{
  ...
  struct Env *e;
  int errono;
  if((errono = env_alloc(&e, curenv->env_id))) {
    return (envid_t)errono;
  }
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
  ...
  if((uint32_t)srcva >= UTOP ||
          ...
          (perm & ~PTE_SYSCALL))
    return -E_INVAL;
  struct PageInfo *pg; pte_t *pte; 
  if(!(pg = page_lookup(srcenv->env_pgdir, srcva, &pte)))
    return -E_INVAL;
  if(!(*pte & PTE_W) && (perm & PTE_W))
    return -E_INVAL;
  if(page_insert(dstenv->env_pgdir, pg, dstva, perm))
    return -E_NO_MEM;
  return 0;
	// LAB 4: Your code here.
...
// Errors are:
//	-E_BAD_ENV if environment envid doesn't currently exist.
//		(No need to check permissions.)11
//	-E_IPC_NOT_RECV if envid is not currently blocked in sys_ipc_recv, 
//		or another environment managed to send first. 11
//	-E_INVAL if srcva < UTOP but srcva is not page-aligned. 11
//	-E_INVAL if srcva < UTOP and perm is inappropriate
//		(see sys_page_alloc). 11
//	-E_INVAL if srcva < UTOP but srcva is not mapped in the caller's
//		address space.  11
//	-E_INVAL if (perm & PTE_W), but srcva is read-only in the
//		current environment's address space. 11
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
  ...
  if(srcva < (void *)UTOP && 
      ...
      return -E_INVAL;
  if(dstenv->env_ipc_dstva < (void *)UTOP && srcva < (void *)UTOP)  {
    r = page_insert(dstenv->env_pgdir, pg, (void *)dstenv->env_ipc_dstva, perm|PTE_P);
    if(r < 0) return r;
    dstenv->env_ipc_perm = perm|PTE_P;
  }
  else 
