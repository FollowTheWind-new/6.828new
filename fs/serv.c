...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if((r = openfile_lookup(envid, req->req_fileid, &op)) < 0) 
		return r;
	if((r = file_read(op->o_file, ret->ret_buf, req->req_n, op->o_fd->fd_offset)) < 0)
		return r;
	op->o_fd->fd_offset += r;
	return r;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	int r; struct OpenFile *op;
	if((r = openfile_lookup(envid, req->req_fileid, &op)) < 0) 
		return r;
	size_t reqn = req->req_n > sizeof(req-> req_buf) ? sizeof(req->req_buf) : req->req_n;
	if((r = file_write(op->o_file, req->req_buf, reqn, op->o_fd->fd_offset)) < 0) 
		return r;
	op->o_fd->fd_offset += r;
	return r;
